...
func TestGetTree(t *testing.T) {
	...
	t.Run("tree via pm ls when lockfile parse fails", func(t *testing.T) {
		dir := t.TempDir()
		oldWd, _ := os.Getwd()
		if err := os.Chdir(dir); err != nil {
			t.Fatal(err)
		...
		// No bun.lock exists — GetTree falls through to getTreeViaPm via the mock.
		a := mkAdapter(t, "my-project\n└── lodash@4.17.21\n", 0)
		tree, err := a.GetTree(context.Background())
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if len(tree.Dependencies) != 1 {
			t.Fatalf("expected 1 dep, got %d", len(tree.Dependencies))
		}
